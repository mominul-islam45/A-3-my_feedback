...
const TopRated_ResCarousal = () => {
  ...
  return (
    <div className='bg-blue py-4 px-10 rounded-xl w-full relative'>
        ...
        {/* Slide Container */}
        <div className='grid lg:grid-cols-4 sm:grid-cols-2 gap-6 py-8 touch-pan-x'>
            {slides.length > 0 && slides[currentIndex].map(({id, name, description, rating, total_ratings, img})=>(
                <Card key={id} className='bg-light rounded-xl basis-full'>
                        <img src={img[0]} alt="Image" className='rounded-xl object-cover h-[200px] aspect-[1.2/1]' />
                    <CardBody className='space-y-1'>
                        <h4 className='text-lg font-semibold cursor-pointer hover:underline underline-offset-3 decoration-2'>{name}</h4>
